...
function render(){
 ...
 $('historyBody').innerHTML=sorted.map(e=>`<tr><td>${new Date(e.ts).toLocaleDateString('pt-PT')}</td><td>${esc(e.game)}</td><td>€${e.result.toFixed(2)}</td><td>${e.mult.toFixed(2)}x</td><td>${e.bonus}</td></tr>`).join('');
...
$('entryForm').addEventListener('submit',e=>{e.preventDefault();entries.push({ts:Date.now(),game:$('game').value.trim(),result:Number($('result').value),mult:Number($('mult').value),bonus:$('bonus').value,note:$('note').value.trim()});e.target.reset();save()});
